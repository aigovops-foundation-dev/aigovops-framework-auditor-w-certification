// issue-attestation — QAGA-issued formal Attestation of AOS Conformance.
//
// Aurora visual language (matches issue-certification):
//   - Layered indigo → emerald → gold header band with "AOS" gold seal
//   - Determination banner (color-coded by verdict)
//   - Risk-tier banner: declared (submitter) vs derived (Agent Council),
//     emerald accent if they agree, violet "underwriting signal" if not
//   - QAGA assessor portrait (real JPEG from agent-portraits bucket if present,
//     gold/violet ring fallback silhouette otherwise) next to a single signature
//     block — this is a one-signer document representing one firm
//   - Per-page Aurora ribbon header on continuation pages
//   - Per-page footer with /verify QR + URL
//
// Signatures: HMAC-SHA256(AUDIT_SIGNING_KEY + "::qaga::" + assessor_id, content_hash)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Issue attestation row via SECURITY DEFINER function (enforces QAGA assignment + independence).
    ...
    if (att?.qaga_assessor_id) {
      const { data: a } = await admin.from("qaga_assessors")
        .select("display_name, qaga_credential_id, user_id").eq("id", att.qaga_assessor_id).single();
      assessorName = a?.display_name ?? assessorName;
      if (a?.qaga_credential_id) assessorRole = `QAGA · ${a.qaga_credential_id}`;
      // Use user_id as portrait filename slug (same convention as agent personas).
    ...
    // Risk tier — declared (submitter) vs derived (Agent Council).
    ...
    // Conformance summary (for findings section).
    const { data: confJson } = await admin.rpc("compute_conformance", { _review_id: reviewId });
    const conf = (confJson ?? {}) as Record<string, unknown>;

    // Top findings for the body.
    const { data: findings } = await admin.from("agent_findings")
      .select("severity, agent_name, title, aos_control_id").eq("review_id", reviewId);

    // Chain-of-custody snapshot (same as cert).
    ...
    // Try to fetch the QAGA's portrait from agent-portraits bucket. Falls back to silhouette.
    ...
    // Aurora palette (matches src/index.css tokens; same values as issue-certification).
    const COLOR = {
    ...
    // Gold seal (top right) — shows AOS version
    ...
    // ---- Risk-tier banner: declared vs derived
    ...
    // ---- Canonical content hash (computed BEFORE signatures are written into PDF)
    ...
    // Vector silhouette fallback (gold ring + violet accent — QAGAs are independent of the cert chiefs).
    ...
    // Single QAGA signature block, gold/violet ring (independent assessor).
    const blockY = y + 30;
    const portraitCx = 82;
    ...
    // Full sig hash + content hash in fine print
    doc.setFont("courier", "normal"); doc.setFontSize(7); setText(COLOR.muted);
